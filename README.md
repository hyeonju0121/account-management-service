...
## ERD
![erd](https://github.com/hyeonju0121/account-management-service/assets/67223214/8485fa3b-b810-40f2-ab5e-95562197f7de)
...
### 1. 회원
- **회원가입**
  - 회원가입 시 아이디, 비밀번호, 간편 비밀번호(6자리), 이름, 이메일이 필요하다.
...
<div markdown="1">    
    ...
    * 관리자
    - 계좌 상품을 등록, 삭제, 수정, 조회 등을 할 수 있어야 한다.
    ...
    * 사용자
    ...
    1. 계좌 상품 종류
    - 현재 계좌 상품 종류는 입출금/정기적금/자유적금 으로 구성되어 있도록 한다.

    2. 계약 기간
    - 계약 기간이 존재하는 경우, 사용자가 해당 상품의 계좌를 개설 시에 자동으로 만기 일자가 저장될 수 있도록 한다.

    3. 이자 지급 날짜
    - 입출금 계좌 상품은 현재 정책상 매월 1일에 이자가 지급될 수 있도록 한다.
    - 자유적금/정기적금 계좌 상품은 만기일에 이자가 지급될 수 있도록 한다.

    4. 매월 납입 횟수
    - 납입 횟수는 제한 없음과 1회로 구성되어 있도록 한다.

  </div>
  
### 관리자
- **계좌 상품 등록**
  - 관리자는 계좌 상품을 등록할 수 있다.
  - 계좌 상품 등록을 위한 계좌 상품 종류, 상품 이름, 상품 설명, 계약 기간, 금리, 이자 지급 방식, 매월 납입 횟수, 매일 최대 저금 금액 등이 필요하다.
  - <details>
    <summary> 다음 사항이 발생하는 경우에는 실패 응답을 반환합니다. </summary>
    
    - 관리자가 로그인을 하지 않은 경우
    - 해당 계좌 상품 이름이 존재하는 경우
    - 정기적금/자유적금 상품을 등록할 때 매월 최대 저금 금액이 입력되지 않은 경우
   
    </details>

- **계좌 상품 조회**
  - 관리자는 계좌 상품을 조회할 수 있다.
  - 상품 이름, 상품 설명, 계약 기간, 금리, 이자 지급 방식, 매월 납입 횟수, 매월 최대 저금 금액, 상품 상태, 상품 이용자 현황 이 응답에 포함되어야 합니다.
  - <details>
    <summary> 다음 사항이 발생하는 경우에는 실패 응답을 반환합니다. </summary>
...
- **계좌 상품 판매 중지**
  - 관리자는 계좌 상품을 판매 중지할 수 있다.
  - 계좌 상품 판매 중지를 위한 계좌 상품 이름이 필요하다.
  - 해지된 계좌 상품 정보가 응답에 포함되어야 합니다.
  - <details>
    <summary> 다음 사항이 발생하는 경우에는 실패 응답을 반환합니다. </summary>
...
- **계좌 상품 수정**
  - 관리자는 계좌 상품을 수정할 수 있다.
  - 수정된 계좌 상품 정보가 응답에 포함되어야 합니다.
  - <details>
    <summary> 다음 사항이 발생하는 경우에는 실패 응답을 반환합니다. </summary>
...
### 회원
- **계좌 상품 조회**
  - 회원은 전체 계좌 상품들을 조회할 수 있다.
  - 상품 이름, 상품 설명, 계약 기간, 금리, 이자 지급 방식, 매월 납입 횟수, 매월 최대 저금 금액 이 응답에 포함되어야 합니다.
  - <details>
    <summary> 다음 사항이 발생하는 경우에는 실패 응답을 반환합니다. </summary>
...
<div markdown="1">    
    ...
    * 자유적금 
      ...
      - 자유적금을 개설할 때, 만기 일자를 입력해야 한다.

    * 정기적금

      - 매월 자동이체될 날짜를 입력받아야 한다.
      - 해당 적금 상품 기간에 맞춰 만기 일자가 자동으로 저장되어야 한다.
